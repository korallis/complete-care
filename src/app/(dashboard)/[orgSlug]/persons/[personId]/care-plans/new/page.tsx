...
import { createCarePlan } from '@/features/care-plans/actions';
...
export async function generateMetadata({
  params,
}: NewCarePlanPageProps): Promise<Metadata> {
  const { personId } = await params;
  const session = await auth();
  if (!session?.user?.activeOrgId) {
    return { title: 'New Care Plan — Complete Care' };
  }
  const person = await getPerson(personId).catch(() => null);
  return {
    title: person
      ? `New Care Plan — ${person.fullName} — Complete Care`
      : 'New Care Plan — Complete Care',
  };
}

export default async function NewCarePlanPage({ params }: NewCarePlanPageProps) {
  ...
  if (!activeMembership || activeMembership.orgSlug !== orgSlug) {
    ...
    redirect(
      `/api/orgs/switch?slug=${orgSlug}&returnTo=/${orgSlug}/persons/${personId}/care-plans/new`,
    );
  }
