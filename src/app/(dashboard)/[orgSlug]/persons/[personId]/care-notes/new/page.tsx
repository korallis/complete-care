...
import { createCareNote } from '@/features/care-notes/actions';
import { hasPermission } from '@/lib/rbac/permissions';
import { CareNoteForm } from '@/components/care-notes/care-note-form';
import type { Role } from '@/lib/rbac/permissions';

interface NewCareNotePageProps {
  params: Promise<{ orgSlug: string; personId: string }>;
}

export async function generateMetadata({
  params,
}: NewCareNotePageProps): Promise<Metadata> {
  const { personId } = await params;
  const session = await auth();
  if (!session?.user?.activeOrgId) {
    return { title: 'New Care Note — Complete Care' };
  }
  const person = await getPerson(personId).catch(() => null);
  return {
    title: person
      ? `New Care Note — ${person.fullName} — Complete Care`
      : 'New Care Note — Complete Care',
  };
}

export default async function NewCareNotePage({
  params,
}: NewCareNotePageProps) {
  ...
  if (!activeMembership || activeMembership.orgSlug !== orgSlug) {
    ...
    redirect(
      `/api/orgs/switch?slug=${orgSlug}&returnTo=/${orgSlug}/persons/${personId}/care-notes/new`,
    );
  }
    ...
    <div className="p-6 max-w-3xl mx-auto">
      ...
      <nav aria-label="Breadcrumb" className="mb-6">
        <ol className="flex items-center gap-2 text-sm text-[oklch(0.55_0_0)]">
          ...
          <li>
            <Link
              href={`/${orgSlug}/persons/${personId}/care-notes`}
              className="hover:text-[oklch(0.35_0.06_160)] transition-colors"
            >
              Care Notes
            </Link>
      ...
      <div className="mb-6">
        ...
        <p className="text-sm text-[oklch(0.55_0_0)] mt-1">
          Record daily observations for {person.fullName}
        </p>
      </div>
