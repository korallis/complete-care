...
import { listBodyMapEntries } from '@/features/documents/actions';
...
export async function generateMetadata({
  params,
}: BodyMapPageProps): Promise<Metadata> {
  const { personId } = await params;
  const session = await auth();
  if (!session?.user?.activeOrgId) {
    return { title: 'Body Map — Complete Care' };
  }
  const person = await getPerson(personId).catch(() => null);
  return {
    title: person
      ? `Body Map — ${person.fullName} — Complete Care`
      : 'Body Map — Complete Care',
  };
}

export default async function BodyMapPage({ params }: BodyMapPageProps) {
  ...
  if (!activeMembership || activeMembership.orgSlug !== orgSlug) {
    ...
    redirect(
      `/api/orgs/switch?slug=${orgSlug}&returnTo=/${orgSlug}/persons/${personId}/body-map`,
    );
  }
  ...
  const initialData = await listBodyMapEntries({
    personId,
    page: 1,
    pageSize: 50,
  });
